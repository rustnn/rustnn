use crate::backends::webnn::api_generated::{
    MlArgMinMaxOptions, MlBatchNormalizationOptions, MlClampOptions, MlContext,
    ...
    MlGraph, MlGraphBuilder, MlGruCellOptions, MlGruOptions, MlGruWeightLayout,
    ...
    MlLstmCellOptions, MlLstmOptions, MlLstmWeightLayout, MlOperand, MlOperandDataType,
    MlOperandDescriptor, MlOperatorOptions, MlPadOptions, MlPaddingMode, MlPool2dOptions,
    MlRecurrentNetworkActivation, MlRecurrentNetworkDirection, MlReduceOptions,
...
pub async fn convert_async(context: &MlContext, graph_info: &GraphInfo) -> Result<MlGraph> {
    let builder =
    ...
    let mut operands = graph_info
        ...
        .map(|(i, o)| {
            ...
            Ok(match o.kind {
                ...
                crate::OperandKind::Output | crate::OperandKind::Intermediate => None,
            })
        })
        ...
        let out = match op {
            ...
            crate::Operation::Lstm {
                ...
                .lstm_with_options(
                    ...
                    *steps,
                    *hidden_size,
                    &to_lstm_options(options, &operands)?,
                )
                .to_vec(),
            crate::Operation::LstmCell {
                ...
                .lstm_cell_with_options(
                    ...
                    get_operand(&operands, *recurrence),
                    get_operand(&operands, *hidden_state),
                    get_operand(&operands, *cell_state),
                    *hidden_size,
                    &to_lstm_cell_options(options, &operands)?,
                )
                .to_vec(),
            crate::Operation::Pad {
                input,
            ...
            crate::Operation::Softmax {
                ...
                vec![builder.softmax_with_options(get_operand(&operands, *input), *axis, &ml_opts)]
            }
            ...
            crate::Operation::QuantizeLinear {
                ...
                vec![builder.quantize_linear_with_options(
                    ...
                    get_operand(
                        &operands,
                        zero_point.expect("WebNN always expects a zero point!"),
                    ),
                    &ml_opts,
                )]
            }
            ...
            crate::Operation::Softplus {
                ...
                vec![builder.softplus_with_options(get_operand(&operands, *input), &ml_opts)]
            }
            crate::Operation::Softsign {
                ...
                vec![builder.softsign_with_options(get_operand(&operands, *input), &ml_opts)]
            }
            crate::Operation::Gelu {
                ...
                vec![builder.gelu_with_options(get_operand(&operands, *input), &ml_opts)]
            }
            ...
            crate::Operation::IsNaN {
                ...
                vec![builder.is_na_n_with_options(get_operand(&operands, *input), &ml_opts)]
            }
            crate::Operation::IsInfinite {
                ...
                vec![builder.is_infinite_with_options(get_operand(&operands, *input), &ml_opts)]
            }
            crate::Operation::RoundEven {
                ...
                vec![builder.round_even_with_options(get_operand(&operands, *input), &ml_opts)]
            }
        };
    ...
    Ok(JsFuture::from(builder.build(&output))
        .await
...
fn to_lstm_options(
    ...
    operands: &[Option<MlOperand>],
) -> Result<MlLstmOptions> {
    let opts = MlLstmOptions::new();
    if let Some(options) = options {
        opts.set_label(&options.label);
        if let Some(bias) = options.bias {
            opts.set_bias(get_operand(operands, bias));
        ...
        if let Some(peephole_weight) = options.peephole_weight {
            opts.set_peephole_weight(get_operand(operands, peephole_weight));
        }
        if let Some(initial_hidden_state) = options.initial_hidden_state {
            opts.set_initial_hidden_state(get_operand(operands, initial_hidden_state));
        }
        if let Some(initial_cell_state) = options.initial_cell_state {
            opts.set_initial_cell_state(get_operand(operands, initial_cell_state));
        }
        opts.set_return_sequence(options.return_sequence);
        if !options.direction.is_empty() {
            opts.set_direction(to_recurrent_network_direction(&options.direction)?);
        }
        if !options.layout.is_empty() {
            opts.set_layout(to_lstm_weight_layout(&options.layout)?);
        }
        if let Some(activations) = &options.activations {
            opts.set_activations(
                &activations
                    .iter()
                    .map(|activation| activation.as_str().into())
                    .collect::<Vec<_>>(),
            );
        }
    }
...
fn to_lstm_cell_options(
    options: &Option<MLLstmCellOptions>,
    operands: &[Option<MlOperand>],
) -> Result<MlLstmCellOptions> {
    let opts = MlLstmCellOptions::new();
    if let Some(options) = options {
        opts.set_label(&options.label);
        if let Some(bias) = options.bias {
            opts.set_bias(get_operand(operands, bias));
...
fn to_pool2d_options(options: &Option<MLPool2dOptions>) -> Result<MlPool2dOptions> {
    ...
    if let Some(options) = options {
        ...
        if !options.output_shape_rounding.is_empty() {
...
fn operand_dtype_to_dtype(dtype: MLOperandDataType) -> Result<MlOperandDataType> {
    Ok(match dtype {
        ...
        MLOperandDataType::Int4 | MLOperandDataType::Uint4 => {
            return Err(WebNNConverterError::UnsupportedDataType(match dtype {
...
fn to_lstm_weight_layout(layout: &str) -> Result<MlLstmWeightLayout> {
    Ok(match layout {
        "iofg" => MlLstmWeightLayout::Iofg,
        "ifgo" => MlLstmWeightLayout::Ifgo,
        _ => {
            return Err(GraphError::ConversionFailed {
                format: "webnn".to_string(),
                reason: format!("Unsupported lstm weight layout: {layout:?}"),
            });
        }
    })
}
