...
| Process aborts with a panic from `ort`: `Failed to load ONNX Runtime dylib: BadVersion { version_str: "1.17.1", path: "onnxruntime.dll" }`, on `MLContext::create` or the CLI `--run-onnx` | `ORT_DYLIB_PATH` is unset or points at a release older than 1.29, and the loader found another `onnxruntime.dll` (on Windows the one in `System32`). This is a panic inside the `ort` crate, not a `rustnn::error::Error`, so it cannot be caught with `?`. Set `ORT_DYLIB_PATH` to the downloaded 1.29 library (`$env:ORT_DYLIB_PATH = "..."` in PowerShell, `export` in bash) and rerun; see [Getting Started](getting-started.md#provide-onnx-runtime) |
